...
wb.xlsx.readFile(fileName).then(() => {
  ...
  for (let i = 2; i < filter_Trucking_Number.length; i++) {
    ...
    if (!(filter_Trucking_Number[i] in truckingNumberToVehicleType)) {
      truckingNumberToVehicleType[thisTruckingNumber] = convertString(filter_transporter[i]) + filter_truck_capacity_in_tons[i] + 'T';
    }
    //grouping route
    if (!(thisTruckingNumber in truckingRoute)) {
      truckingRoute[thisTruckingNumber] = [filter_shipto_party_number[i]];
    } else if (!(truckingRoute[thisTruckingNumber].includes(filter_shipto_party_number[i]))) {
  ...
  outputDict.solutions.forEach((solution) => {
    solution.routes.forEach((route) => {
      ...
      for (const dis of inputData['matrixConfig']['DC']['distanceBilling']['matrix']) {
        if ((dis['typeOfCustomer'] in distanceDict) && (dis['typeOfVehicle'] === thisVehicle)) {
          if (dis['value'] <= 0) {
      ...
      for (let m of inputData['matrixConfig']['VC']['mainFee']['matrix']) {
        if ((m['typeOfCustomer'] === furthestLoc) && (m['typeOfVehicle'] === thisVehicle)) {
          mainFee = m['value']
          break
        }
      }
      ...
      if (numberOfAdditionalFee > 0) {
        for (let a of inputData['matrixConfig']['VC']['additionalFee']['matrix']) {
